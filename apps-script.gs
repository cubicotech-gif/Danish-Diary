 ...
 *   - "Ledger"  — entryId, personId, date, type, amount, description,
 *                 category, dueDate, createdAt
 *
...
function readPeople_() {
  ...
  return values
    .map((r, i) => ({
      ...
      currency: normCurrency_(r[P.currency - 1]),
...
function readLedger_() {
  ...
  return values
    .map((r, i) => ({
      ...
      category: normCategory_(r[L.category - 1]),
      dueDate: r[L.dueDate - 1] instanceof Date ? r[L.dueDate - 1] : null,
...
function buildDashboard_() {
  ...
  // Outstanding is grouped by currency since amounts in different
  // currencies can't be summed into one number.
  ...
  const recent = ledger
    ...
    .map((e) => ({
      ...
      category: e.category,
      dueDate: e.dueDate ? Utilities.formatDate(e.dueDate, tz, 'yyyy-MM-dd') : ''
    }));
...
function normCurrency_(c) {
  ...
  return ALLOWED_CURRENCIES.indexOf(s) >= 0 ? s : DEFAULT_CURRENCY;
}
